...
def get_dashboard_stats():
    try:
        ...
        cursor.execute("SELECT COUNT(*) as active FROM users WHERE last_login > %s", (thirty_days_ago,))
...
def get_documents():
    try:
        ...
        # Get documents
        query = f"""
            SELECT ud.id, ud.user_id, u.email as user_email, ud.file_name, 
                   ud.file_size, ud.document_type, ud.created_at, ud.download_count
            FROM user_documents ud 
            JOIN users u ON ud.user_id = u.user_id 
            WHERE {where_clause}
        ...
        for doc in documents:
            ...
            doc['file_size'] = f"{doc['file_size'] / 1024 / 1024:.1f} MB"
...
def get_payments():
    try:
        ...
        # Get payments
        query = f"""
            SELECT p.id, p.user_id, u.email as user_email, p.amount, p.status, 
                   p.payment_method, p.reference, p.created_at, p.processed_at
            FROM payments p 
            JOIN users u ON p.user_id = u.user_id 
            WHERE {where_clause}
...
def get_system_logs():
    try:
        ...
        # Get logs
        query = f"""
            SELECT id, level, message, source, user_id, created_at
            FROM system_logs 
            WHERE {where_clause}
            ORDER BY created_at DESC
            LIMIT %s OFFSET %s
