...
class AdminService {
  ...
  // Updated: Fixed all API paths to use /admin prefix

  // User Management
  async getUsers(filters: UserFilters = {}): Promise<UsersResponse> {
    try {
      // First try the simple test endpoint
      const response = await adminApi.get('/simple-admin/users');
  ...
  async updateUser(userId: number, updates: UserUpdateForm): Promise<void> {
    try {
      await adminApi.put(`/admin/users/${userId}`, updates);
    } catch (error) {
      throw this.handleError(error);
    }
  }
  ...
  async getUserDocuments(userId: number, page: number = 1, perPage: number = 20): Promise<DocumentsResponse> {
    try {
      ...
      const response = await adminApi.get(`/admin/users/${userId}/documents?${params.toString()}`);
      return response.data;
    } catch (error) {
      throw this.handleError(error);
    }
  ...
  async downloadDocument(reference: string): Promise<Blob> {
    try {
      const response = await adminApi.get(`/admin/documents/${reference}/download`, {
        responseType: 'blob'
      });
      return response.data;
    } catch (error) {
  ...
  async approvePayment(paymentId: number, approval: PaymentApprovalForm): Promise<void> {
    try {
      await adminApi.put(`/admin/payments/${paymentId}/approve`, approval);
    } catch (error) {
      throw this.handleError(error);
    }
  }
  ...
  async getPaymentDocument(paymentId: number): Promise<Document> {
    try {
      const response = await adminApi.get(`/admin/payments/${paymentId}/document`);
  ...
  async getSystemLogs(filters: LogFilters = {}): Promise<SystemLogsResponse> {
    try {
      ...
      const response = await adminApi.get(`/admin/system/logs?${params.toString()}`);
      return response.data;
    } catch (error) {
      throw this.handleError(error);
    }
  ...
  async sendNotification(notification: NotificationForm): Promise<void> {
    try {
      await adminApi.post(`/admin/notifications/send`, notification);
    } catch (error) {
      throw this.handleError(error);
    }
  }
  ...
  async getNotifications(page: number = 1, perPage: number = 50, status?: string): Promise<NotificationsResponse> {
    try {
      ...
      const response = await adminApi.get(`/admin/notifications?${params.toString()}`);
    return response.data;
    } catch (error) {
      throw this.handleError(error);
    }
  ...
  async getAnalyticsStats(period: string = '30d'): Promise<AnalyticsStats> {
    try {
      ...
      const response = await adminApi.get(`/admin/analytics/stats?${params.toString()}`);
    return response.data;
    } catch (error) {
      throw this.handleError(error);
    }
  ...
  async exportAnalytics(options: ExportOptions): Promise<ExportResponse> {
    try {
      ...
      const response = await adminApi.get(`/admin/analytics/export?${params.toString()}`);
      return response.data;
    } catch (error) {
      throw this.handleError(error);
    }
