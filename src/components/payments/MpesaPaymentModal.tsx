...
export const MpesaPaymentModal: React.FC<MpesaPaymentModalProps> = ({
  ...
  const [submission, setSubmission] = useState<MpesaFormSubmission | null>(null);
  const [pollingInterval, setPollingInterval] = useState<NodeJS.Timeout | null>(null);
  const [retryCount, setRetryCount] = useState(0);
  const [maxRetries] = useState(3);

  const amount = mpesaService.getPricing(documentType);
  ...
  const validateForm = (): boolean => {
    ...
    if (!mpesaService.validatePhoneNumber(phone)) {
  ...
  const initiatePayment = async () => {
    ...
    try {
      const response = await mpesaService.submitFormWithPayment({
        form_data: formData,
  ...
  const startStatusPolling = (submissionId: number) => {
    ...
    const interval = setInterval(async () => {
      try {
        const response = await mpesaService.checkSubmissionStatus(submissionId);
        
        if (response.success && response.submission) {
          setSubmission(response.submission);
          
          if (response.submission.status === 'completed' || response.submission.status === 'email_sent') {
          ...
          } else if (response.submission.status === 'paid') {
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-600">
          Pay with M-Pesa to generate your {mpesaService.formatDocumentType(documentType)}
        </p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          <div className="flex justify-between items-center">
            ...
            <span className="font-medium">{mpesaService.formatDocumentType(documentType)}</span>
          </div>
  ...
  const renderProcessingStatus = () => (
      ...
      {submission?.status === 'paid' && (
          ...
          <ul className="list-disc list-inside text-blue-700 mt-2 space-y-1">
            <li>Generating your {mpesaService.formatDocumentType(documentType)}</li>
      ...
      {submission?.mpesa_receipt_number && (
        <div className="bg-green-50 rounded-lg p-4 text-sm">
          ...
          <p className="text-green-700 mt-1">
            Receipt: {submission.mpesa_receipt_number}
          </p>
            </div>
      )}
          </div>
  ...
  const renderCompletedStatus = () => (
      ...
      <h3 className="text-lg font-semibold text-green-800">Order Complete!</h3>
            <p className="text-gray-600">
        Your {mpesaService.formatDocumentType(documentType)} has been generated and sent to your email
            </p>
      ...
      {submission?.mpesa_receipt_number && (
        <div className="bg-gray-50 rounded-lg p-3 text-sm">
          <p className="font-medium text-gray-800">Payment Receipt:</p>
